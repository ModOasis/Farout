...
public class BulletEntity extends AbstractArrow implements ItemSupplier {
	public static final ItemStack PROJECTILE_ITEM = new ItemStack(FaroutModItems.BULLET_ITEM.get());

	public BulletEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(FaroutModEntities.BULLET.get(), world);
	}
	...
	public static BulletEntity shoot(Level world, LivingEntity entity, RandomSource source) {
		return shoot(world, entity, source, 10f, 0.1, 0);
	}
	...
	public static BulletEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		BulletEntity entityarrow = new BulletEntity(FaroutModEntities.BULLET.get(), entity, world);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("farout:gun_fire")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static BulletEntity shoot(LivingEntity entity, LivingEntity target) {
		BulletEntity entityarrow = new BulletEntity(FaroutModEntities.BULLET.get(), entity, entity.level());
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("farout:gun_fire")), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
