...
public class ProxmaiEffectsProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != ProxmaiEffectsProcedure.amount || seed != ProxmaiEffectsProcedure.seed) {
			ProxmaiEffectsProcedure.amount = amount;
			ProxmaiEffectsProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != ProxmaiEffectsProcedure.effects) {
				ProxmaiEffectsProcedure.effects = effects;
				ProxmaiEffectsProcedure.customSky = (List<Consumer<Object[]>>) effects.getField("customSky").get(null);
			}
			ProxmaiEffectsProcedure.customSky.add(consumer);
		} catch (Exception e) {
		}
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("farout:proxmai"))) {
			...
			renderSun(150, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
			RenderSystem.setShaderTexture(0, new ResourceLocation((FaroutModVariables.MapVariables.get(world).ModID + ":textures/" + "vulcan" + ".png")));
			renderTexture(25, 90, (float) (world.dayTime() * 0.03), 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
		}
	}
}
