...
public class SkaffronDebrisSwarmEffectsProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != SkaffronDebrisSwarmEffectsProcedure.amount || seed != SkaffronDebrisSwarmEffectsProcedure.seed) {
			SkaffronDebrisSwarmEffectsProcedure.amount = amount;
			SkaffronDebrisSwarmEffectsProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != SkaffronDebrisSwarmEffectsProcedure.effects) {
				SkaffronDebrisSwarmEffectsProcedure.effects = effects;
				SkaffronDebrisSwarmEffectsProcedure.customSky = (List<Consumer<Object[]>>) effects.getField("customSky").get(null);
			}
			SkaffronDebrisSwarmEffectsProcedure.customSky.add(consumer);
		} catch (Exception e) {
		}
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		if (entity == null)
			return;
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("farout:deleted_mod_element"))) {
			renderSky(true, true, false, false, true, true);
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
			RenderSystem.setShaderTexture(0, new ResourceLocation((FaroutModVariables.MapVariables.get(world).ModID + ":textures/" + "tuqeg" + ".png")));
			renderTexture(100, 90, 90, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
		}
	}
}
