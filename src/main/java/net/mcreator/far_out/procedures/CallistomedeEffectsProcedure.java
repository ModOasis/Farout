...
public class CallistomedeEffectsProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != CallistomedeEffectsProcedure.amount || seed != CallistomedeEffectsProcedure.seed) {
			CallistomedeEffectsProcedure.amount = amount;
			CallistomedeEffectsProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != CallistomedeEffectsProcedure.effects) {
				CallistomedeEffectsProcedure.effects = effects;
				CallistomedeEffectsProcedure.customSky = (List<Consumer<Object[]>>) effects.getField("customSky").get(null);
			}
			CallistomedeEffectsProcedure.customSky.add(consumer);
		} catch (Exception e) {
		}
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		if (entity == null)
			return;
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("farout:callistomede"))) {
			renderSky(true, true, false, false, true, true);
			RenderSystem.defaultBlendFunc();
			RenderSystem.setShaderTexture(0, new ResourceLocation((FaroutModVariables.MapVariables.get(world).ModID + ":textures/" + "jupitermask" + ".png")));
			renderTexture(60, 90, 225, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
			RenderSystem.setShaderTexture(0, new ResourceLocation((FaroutModVariables.MapVariables.get(world).ModID + ":textures/" + "jupiter" + ".png")));
			renderTexture(60, 90, 225, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
			RenderSystem.setShaderTexture(0, new ResourceLocation((FaroutModVariables.MapVariables.get(world).ModID + ":textures/" + "sandosrealistic" + ".png")));
