...
public class VenusionEffectsProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != VenusionEffectsProcedure.amount || seed != VenusionEffectsProcedure.seed) {
			VenusionEffectsProcedure.amount = amount;
			VenusionEffectsProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != VenusionEffectsProcedure.effects) {
				VenusionEffectsProcedure.effects = effects;
				VenusionEffectsProcedure.customSky = (List<Consumer<Object[]>>) effects.getField("customSky").get(null);
			}
			VenusionEffectsProcedure.customSky.add(consumer);
		} catch (Exception e) {
		}
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("farout:venasion"))) {
			...
			RenderSystem.setShaderTexture(0, new ResourceLocation((FaroutModVariables.MapVariables.get(world).ModID + ":textures/" + "zanavos" + ".png")));
			renderTexture(180, 90, 225, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
		}
	}
}
