...
public class HelusEffectsProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != HelusEffectsProcedure.amount || seed != HelusEffectsProcedure.seed) {
			HelusEffectsProcedure.amount = amount;
			HelusEffectsProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != HelusEffectsProcedure.effects) {
				HelusEffectsProcedure.effects = effects;
				HelusEffectsProcedure.customSky = (List<Consumer<Object[]>>) effects.getField("customSky").get(null);
			}
			HelusEffectsProcedure.customSky.add(consumer);
		} catch (Exception e) {
		}
	}
