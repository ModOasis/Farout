...
public class KalasmosLighingProcedure {
	...
	public static void computeLightColor(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != KalasmosLighingProcedure.effects) {
				KalasmosLighingProcedure.effects = effects;
				KalasmosLighingProcedure.customLights = (List<Consumer<Object[]>>) effects.getField("customLights").get(null);
			}
			KalasmosLighingProcedure.customLights.add(consumer);
		} catch (Exception e) {
		}
	}
