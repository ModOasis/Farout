...
public class CarbosEffectsProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != CarbosEffectsProcedure.amount || seed != CarbosEffectsProcedure.seed) {
			CarbosEffectsProcedure.amount = amount;
			CarbosEffectsProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != CarbosEffectsProcedure.effects) {
				CarbosEffectsProcedure.effects = effects;
				CarbosEffectsProcedure.customSky = (List<Consumer<Object[]>>) effects.getField("customSky").get(null);
			}
			CarbosEffectsProcedure.customSky.add(consumer);
		} catch (Exception e) {
		}
	}
