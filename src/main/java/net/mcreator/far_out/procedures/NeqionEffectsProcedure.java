...
public class NeqionEffectsProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != NeqionEffectsProcedure.amount || seed != NeqionEffectsProcedure.seed) {
			NeqionEffectsProcedure.amount = amount;
			NeqionEffectsProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != NeqionEffectsProcedure.effects) {
				NeqionEffectsProcedure.effects = effects;
				NeqionEffectsProcedure.customSky = (List<Consumer<Object[]>>) effects.getField("customSky").get(null);
			}
			NeqionEffectsProcedure.customSky.add(consumer);
		} catch (Exception e) {
		}
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("farout:neqion"))) {
			...
			renderSun(120, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
			RenderSystem.setShaderTexture(0, new ResourceLocation((FaroutModVariables.MapVariables.get(world).ModID + ":textures/" + "quatu" + ".png")));
			renderTexture(30, 90, 225, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
		}
	}
}
