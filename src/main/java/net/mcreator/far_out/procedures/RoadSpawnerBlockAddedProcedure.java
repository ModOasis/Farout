...
public class RoadSpawnerBlockAddedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (!(Mth.nextInt(RandomSource.create(), 1, 10) == 9)) {
			if (world instanceof ServerLevel _serverworld) {
				StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("farout", "trihassian_city_road_apartment"));
				if (template != null) {
