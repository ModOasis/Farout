...
public class FairingHabProcedure {
	public static String execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity instanceof Player _plr0 && _plr0.containerMenu instanceof DesigningWorkbenchGUIMenu) {
			...
			ReturnValue = ""
					+ ((entity instanceof Player _plrSlotItem && _plrSlotItem.containerMenu instanceof Supplier _splr && _splr.get() instanceof Map _slt ? ((Slot) _slt.get(0)).getItem() : ItemStack.EMPTY).getOrCreateTag().getDouble("HabTime"));
		}
		return ReturnValue;
	}
}
