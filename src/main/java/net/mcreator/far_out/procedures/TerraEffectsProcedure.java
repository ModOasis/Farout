...
public class TerraEffectsProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != TerraEffectsProcedure.amount || seed != TerraEffectsProcedure.seed) {
			TerraEffectsProcedure.amount = amount;
			TerraEffectsProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != TerraEffectsProcedure.effects) {
				TerraEffectsProcedure.effects = effects;
				TerraEffectsProcedure.customSky = (List<Consumer<Object[]>>) effects.getField("customSky").get(null);
			}
			TerraEffectsProcedure.customSky.add(consumer);
		} catch (Exception e) {
		}
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		if ((entity.level().dimension()) == Level.OVERWORLD) {
			...
			renderTexture(25, 90, (float) (world.dayTime() * 0.005), 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
			RenderSystem.setShaderTexture(0, new ResourceLocation((FaroutModVariables.MapVariables.get(world).ModID + ":textures/" + "lunon" + ".png")));
			renderTexture(25, 90, (float) (world.dayTime() * 0.005), 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
